function showSuggestions(input) {
    ...
    if (!input) {
        ...
        // Example static data, replace with your data source or API call
        let suggestions = [
        ...
        if (filtered.length) {
            suggestionsBox.innerHTML = filtered.map(item => `<div onclick="selectSuggestion('${item}')">${item}</div>`).join('');
